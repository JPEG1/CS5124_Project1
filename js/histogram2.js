class Histogram2 {
    ...
    initVis() {
      ...
      let filteredDisplayData = vis.data.filter(d => d[selectedXAxis] !== -1);
      ...
      const x = d3.scaleLinear()
        .domain([0, d3.max(filteredDisplayData, function(d) {return +d[selectedXAxis]})+10])     // can use this instead of 1000 to have the max of data: d3.max(data, function(d) { return +d.price })
        .range([0, vis.width]);
      ...
      svg.selectAll("rect")
        ...
        .on("mouseover", function(event, d) {
            ...
            histgram2_tooltip
                ...
                .html(`
                    ...
                    <div><i>${TooltipData.pre_x_unit}${d.x0}${TooltipData.post_x_unit}-${TooltipData.pre_x_unit}${d.x1}${TooltipData.post_x_unit}</i></div>
                `);
            histgram2_tooltip.transition()
                .duration(200)
                .style("opacity", 0.9)
        })
        ...
        const histgram2_tooltip = d3.select("#histogram2-tooltip");
    }
    ...
    updateVis() {
        ...
        d3.selectAll("#histogram2 svg").remove();
